...
public class SingleTable extends QueryTable implements Iterator<Row> {
  ...
  public void PrepareNext() {
    ...
    if (mLogicSelect == null) {
      ...
      if (the_condition == null) {
        ...
        // condition是常量比较
        if (left_comparer.mType != ComparerType.COLUMN
            && right_comparer.mType != ComparerType.COLUMN) {
        ...
        else if (left_comparer.mType == ComparerType.COLUMN
            && right_comparer.mType != ComparerType.COLUMN
        ...
        } else if (right_comparer.mType == ComparerType.COLUMN
            && left_comparer.mType != ComparerType.COLUMN
            && the_condition.mType == ConditionType.EQ) {
          String primary_key_name = mTable.GetPrimaryName();
  ...
  /** 描述：将待比较元素强制类型转换成主键元素 参数：待比较元素 返回：主键元素 */
  ...
  /** 描述：用table的cache机制加速查找，仅限查找主键，单一逻辑，=的情况 参数：待查找的主键value 返回：无 */
  ...
  /** 描述：对于一般情况的查找 参数：待查找的主键value 返回：无 */
