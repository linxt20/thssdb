        ...
        for(Column column : columns) {
            if(column.getName().equals(columnName)) {
                ...
                for (Row row : oldTable) {
                    System.out.println("row.getEntries().get(i).toString(): " + row.getEntries().get(i).toString());
                    // 判断是否符合condition，符合则insert
                    if(type == ColumnType.INT || type == ColumnType.LONG || type == ColumnType.FLOAT || type == ColumnType.DOUBLE){
